...
- Если **хотя бы одна метрика** в состоянии **`critical`** (порог `ratio > 2` относительно мягкой зоны), общий **`health_status` всегда `critical`**, даже если индекс из‑за остальных нормальных полей ещё высокий: для оператора критичный узел не должен теряться на фоне «хорошего среднего».
- Иначе базово по **интегральному индексу**: ≥ 80 — `normal`, 50–79 — `attention`, **ниже 50** — `critical`.
- Если есть только **warning** по метрикам (без `critical`) и по индексу было бы `normal`, итог — минимум **`attention`**.
